function [waves] = SlowWaveDetect(handles,eeg)
Ff = handles.filtered_matrix(:,eeg);
fs = handles.hertz;
epochl = str2double(get(handles.epochLength,'String'));
if isfield(handles,'num_of_files')
    mbar = waitbar(0,['Calculating Wave Characteristics of EEG ',num2str(eeg-1),' in file ',num2str(handles.current_file),' of ',num2str(handles.num_of_files)]);
elseif ishandle(handles.bar)
    waitbar(11/16,handles.bar,['Calculating Wave Characteristics of EEG ',num2str(eeg-1)]);
else
    bar = waitbar(0,['Calculating Wave Characteristics of EEG ',num2str(eeg-1)]);
end
zermat=zeros(1,length(Ff));
...
% uppeaks=find(imregionalmax(Ff)>0); % Find the indices of max voltages
% troughs=find(imregionalmin(Ff)>0); % Find the indices of min voltages
uppeaks(Ff(uppeaks)<0)=[]; % Eliminate any negative local maxima
troughs(Ff(troughs)>0)=[]; % Eliminate any positive local minima
...
for i=1:length(tro)-1
    ...
    if sn==1 && sp==1 % If they are both peaks
        if exist('bar','var')
            waitbar(i/(length(tro)-1),bar,['Calculating Wave Characteristics of EEG ',num2str(eeg-1)]);
        elseif exist('mbar','var')
            waitbar((i/(length(tro)-1)),mbar,['Calculating Wave Characteristics of EEG ',num2str(eeg-1),' in file ',num2str(handles.current_file),' of ',num2str(handles.num_of_files)]);
        else
            waitbar(5/16*(i/(length(tro)-1))+11/16,handles.bar,['Calculating Wave Characteristics of EEG ',num2str(eeg-1)]);
        end
        ...
        wave=Ff(wavest:waveend); % Define the wave
        ...
        nump=imregionalmax(wave); % Find the local maxima within the wave.
